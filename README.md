...
| `.\az-vm.cmd task --run-vm-update 10002 --group <resource-group> --vm-name <vm-name> --windows` | `--run-vm-update`, platform | Runs one update task directly | Isolated guest fix | Uses the SSH task runner against one managed VM; task-signaled immediate restarts still run, Windows Store-backed app tasks also append `10003-create-shortcuts-public-desktop` in the same isolated rerun so Public Desktop shortcuts refresh immediately, and the workflow-only final vm-update restart still does not run. |
...
- Windows Store-backed isolated `task --run-vm-update` reruns append `10003-create-shortcuts-public-desktop` automatically after the selected Store app task so Public Desktop shortcuts refresh in the same diagnostic run.
