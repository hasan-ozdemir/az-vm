...
Assert-True -Condition (-not ($readmeText -match [regex]::Escape('app-states/<task-name>'))) -Message 'README.md must not keep the retired stage-local app-states path.'
Assert-True -Condition (-not ($agentsText -match [regex]::Escape('app-states/<task-name>'))) -Message 'AGENTS.md must not keep the retired stage-local app-states path.'
...
Assert-True -Condition (($promptHistoryText -match '(?m)^### \d{4}-\d{2}-\d{2} \d{2}:\d{2} UTC$')) -Message 'docs/prompt-history.md must contain UTC timestamp headings.'
