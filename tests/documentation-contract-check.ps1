...
Assert-True -Condition ($readmeText -match [regex]::Escape('Windows `vm-update` begins after one planned restart at the start of Step 6, and the workflow performs one more restart automatically before `vm-summary` when any update task requests reboot.')) -Message 'README.md must document the wrapped Windows vm-update restart behavior.'
