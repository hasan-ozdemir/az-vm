...
Assert-True -Condition ($agentsText -match [regex]::Escape('Azure-touching commands support `--subscription-id` plus `-s`; resolution precedence is CLI override -> `.env` `azure_subscription_id` -> active Azure CLI subscription.')) -Message 'AGENTS.md must define subscription precedence.'
Assert-True -Condition ($agentsText -match [regex]::Escape('Azure-touching commands require an authenticated Azure CLI session; help, README, and runtime errors must say `az login` is required.')) -Message 'AGENTS.md must define the az login requirement.'
...
Assert-True -Condition ($readmeText -match [regex]::Escape('Azure CLI sign-in is strictly required for Azure-touching commands. Run `az login`')) -Message 'README.md must document the strict az login requirement.'
Assert-True -Condition ($readmeText -match [regex]::Escape('Azure subscription selection precedence is: CLI `--subscription-id` / `-s` -> `.env` `azure_subscription_id` -> active Azure CLI subscription.')) -Message 'README.md must document subscription precedence.'
Assert-True -Condition ($readmeText -match [regex]::Escape('`azure_subscription_id`: optional repo-local default Azure subscription id for Azure-touching commands.')) -Message 'README.md must document azure_subscription_id.'
Assert-True -Condition ($readmeText -match [regex]::Escape('`-s`, `--subscription-id=<subscription-guid>`: target Azure subscription for every Azure-touching command; successful CLI usage also writes `azure_subscription_id` into `.env`.')) -Message 'README.md must document the subscription global option.'
...
Assert-True -Condition ($readmeText -match [regex]::Escape('Azure-read-only output; `--subscription-id` / `-s` only changes the subscription context and persists `azure_subscription_id` when it comes from the CLI')) -Message 'README.md must explain list subscription persistence clearly.'
