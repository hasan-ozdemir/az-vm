...
Assert-True -Condition ($readmeText -match [regex]::Escape('Reboot-signaling `vm-init` and `vm-update` tasks restart the VM immediately, wait for recovery, and then continue with the next task. Windows `vm-update` also performs one additional final restart before `vm-summary`.')) -Message 'README.md must document the task-immediate and final Windows vm-update restart behavior.'
