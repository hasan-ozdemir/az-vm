...
Write-Host "Update task started: install-itunes"
...
if (-not [string]::IsNullOrWhiteSpace([string]$existingExe)) {
    ...
    Write-Host "install-itunes-completed"
    Write-Host "Update task completed: install-itunes"
    return
}
...
Write-Host "Running: winget install --id Apple.iTunes --accept-source-agreements --accept-package-agreements --silent --disable-interactivity --force"
& $wingetExe install --id Apple.iTunes --accept-source-agreements --accept-package-agreements --silent --disable-interactivity --force
