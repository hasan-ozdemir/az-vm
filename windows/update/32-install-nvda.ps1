...
Write-Host "Update task started: install-nvda"
...
if (-not [string]::IsNullOrWhiteSpace([string]$existingExe)) {
    ...
    Write-Host "install-nvda-completed"
    Write-Host "Update task completed: install-nvda"
    return
}
...
Write-Host "Running: winget install --id NVAccess.NVDA --accept-source-agreements --accept-package-agreements --silent --disable-interactivity --force"
& $wingetExe install --id NVAccess.NVDA --accept-source-agreements --accept-package-agreements --silent --disable-interactivity --force
