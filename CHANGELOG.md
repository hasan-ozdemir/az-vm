...
- Added Windows `vm-update` task `39-auto-start-apps` to mirror this local machine's enabled startup-app profile onto the guest VM for supported repo-installed apps by resolving the host startup state at runtime, passing it into task tokens, and creating machine startup shortcuts for the matching guest applications.
...
- Extended the late Windows validation path so `29-health-snapshot` now also inventories the mirrored auto-start shortcut contract that `39-auto-start-apps` writes under the machine Startup folder.
