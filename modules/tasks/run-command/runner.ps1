...
function New-AzVmRunCommandTaskWrapperScript {
    ...
    [void]$builder.AppendLine('        $taskScriptBase64 = [string]$ScriptBase64')
    [void]$builder.AppendLine('        $nestedScript = ''$ErrorActionPreference = "Stop"'' + [Environment]::NewLine + ''$ProgressPreference = "SilentlyContinue"'' + [Environment]::NewLine + (''$taskScriptBase64 = '''''''' + $taskScriptBase64 + '''''''''') + [Environment]::NewLine + ''try {'' + [Environment]::NewLine + ''    $decodedScript = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($taskScriptBase64))'' + [Environment]::NewLine + ''    $taskScriptBlock = [scriptblock]::Create($decodedScript)'' + [Environment]::NewLine + ''    & $taskScriptBlock'' + [Environment]::NewLine + ''    [Console]::Out.WriteLine("AZ_VM_NESTED_RESULT:success")'' + [Environment]::NewLine + ''    exit 0'' + [Environment]::NewLine + ''} catch {'' + [Environment]::NewLine + ''    $nestedFailure = [string]$_.Exception.Message'' + [Environment]::NewLine + ''    if (-not [string]::IsNullOrWhiteSpace([string]$nestedFailure)) { [Console]::Error.WriteLine($nestedFailure) }'' + [Environment]::NewLine + ''    [Console]::Out.WriteLine("AZ_VM_NESTED_RESULT:error")'' + [Environment]::NewLine + ''    exit 1'' + [Environment]::NewLine + ''}''')
    ...
    [void]$builder.AppendLine('        $process = Start-Process -FilePath ''powershell.exe'' -ArgumentList @(''-NoProfile'',''-NonInteractive'',''-ExecutionPolicy'',''Bypass'',''-File'', $nestedScriptPath) -RedirectStandardOutput $stdoutPath -RedirectStandardError $stderrPath -PassThru -WindowStyle Hidden')
    ...
    [void]$builder.AppendLine('        }')
    [void]$builder.AppendLine('        foreach ($artifactPath in @($nestedScriptPath, $stdoutPath, $stderrPath)) {')
