...
function New-AzVmRunCommandTaskWrapperScript {
    ...
    [void]$builder.AppendLine('        $commandOutput = & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $tempPath 2>&1')
    [void]$builder.AppendLine('        $commandOutputLines = @($commandOutput | ForEach-Object { [string]$_ })')
    [void]$builder.AppendLine('        $commandExitCode = [int]$LASTEXITCODE')
    [void]$builder.AppendLine('        foreach ($line in @($commandOutputLines)) {')
    ...
    [void]$builder.AppendLine('        if ($commandExitCode -ne 0) { throw ("Task exited with code {0}." -f $commandExitCode) }')
