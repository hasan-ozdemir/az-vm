...
function Show-AzVmCommandHelpOverview {
    ...
    Write-Host "  task    List discovered init/update tasks, or save/restore one task app-state payload."
    ...
    Write-Host "  az-vm task --save-app-state --vm-update-task=115 --group=<resource-group> --vm-name=<vm-name>"
    Write-Host "  az-vm task --restore-app-state --vm-update-task=115 --group=<resource-group> --vm-name=<vm-name>"
...
function Show-AzVmCommandHelpDetailed {
    ...
    if ([string]::IsNullOrWhiteSpace($topicName)) {
        ...
        Write-Host "  -s, --subscription-id    # all Azure-touching commands, plus task save/restore"
        Write-Host "  -h, --help"
        Write-Host "  Azure CLI sign-in via 'az login' is required for all Azure-touching commands."
        Write-Host ""
        ...
        Write-Host "  create  : supports --subscription-id, --step-to, --step-from, --step, --vm-name, --vm-region, --vm-size"
        ...
        Write-Host "  task    : supports --list, inventory filters, and live --save-app-state/--restore-app-state maintenance"
    ...
    switch ($topicName) {
        ...
        Write-Host "  az-vm create --step=<step> [--subscription-id=<subscription-id>]"
            ...
            Write-Host "Examples:"
            Write-Host "  az-vm create --auto --windows --vm-name=<vm-name> --vm-region=<azure-region> --vm-size=<vm-sku> -s <subscription-guid>"
            Write-Host "  az-vm delete --target=group --group=<resource-group> --yes --subscription-id=<subscription-guid>"
            Write-Host "  az-vm create --auto --windows --vm-name=<vm-name> --vm-region=<azure-region> --vm-size=<vm-sku> --subscription-id=<subscription-guid>"
        ...
        'task' {
            ...
            Write-Host "Description: list discovered init/update tasks, or save/restore one task-owned app-state payload against a live VM."
            Write-Host "Usage:"
            Write-Host "  az-vm task --list [--vm-init] [--vm-update] [--disabled] [--windows|--linux] [--perf]"
            Write-Host "  az-vm task --save-app-state --vm-init-task=<task-number|task-name> [--group=<resource-group>] [--vm-name=<vm-name>] [--subscription-id=<subscription-id>] [--windows|--linux] [--perf]"
            Write-Host "  az-vm task --save-app-state --vm-update-task=<task-number|task-name> [--group=<resource-group>] [--vm-name=<vm-name>] [--subscription-id=<subscription-id>] [--windows|--linux] [--perf]"
            Write-Host "  az-vm task --restore-app-state --vm-init-task=<task-number|task-name> [--group=<resource-group>] [--vm-name=<vm-name>] [--subscription-id=<subscription-id>] [--windows|--linux] [--perf]"
            Write-Host "  az-vm task --restore-app-state --vm-update-task=<task-number|task-name> [--group=<resource-group>] [--vm-name=<vm-name>] [--subscription-id=<subscription-id>] [--windows|--linux] [--perf]"
            ...
            Write-Host "  az-vm task --save-app-state --vm-update-task=115 --group=<resource-group> --vm-name=<vm-name> -s <subscription-guid>"
            Write-Host "  az-vm task --restore-app-state --vm-update-task=115 --group=<resource-group> --vm-name=<vm-name>"
            Write-Host "Notes: --list is local-only and scans tracked plus local task trees with the same discovery rules used by init/update execution. --save-app-state and --restore-app-state are Azure-touching live-maintenance paths: they require Azure CLI sign-in via 'az login', select one managed VM target like exec, and read or deploy only `.../<stage>/app-states/<task-name>/app-state.zip`. Save overwrites the zip in place and cleanly skips when no capture coverage exists. Restore fails cleanly when the requested zip is missing or invalid."
            return
        }
        'move' {
