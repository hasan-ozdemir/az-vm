    ...
    switch ($topicName) {
        ...
        'create' {
            Write-AzVmHelpLines @(
                ...
                'Notes: create always targets a fresh managed resource group and fresh managed resources for full runs. Interactive mode prompts for Azure subscription when --subscription-id is omitted, asks for VM OS type first when --windows/--linux is omitted, proposes the next global gX name plus globally unique nX resource ids, and asks yes/no/cancel review checkpoints only for group, vm-deploy, vm-init, and vm-update. Auto mode runs from the fully resolved selection set: CLI overrides win, otherwise .env SELECTED_* values plus the platform VM defaults must resolve platform, VM name, Azure region, and VM size. When a partial create window starts at vm-init, vm-update, or vm-summary, az-vm reuses the existing managed target created earlier instead of generating a new gX resource group name. Reboot-signaling vm-init and vm-update tasks restart the VM immediately, wait for recovery, and then continue with the next task. Windows vm-update also ends with one additional final restart before vm-summary. vm-summary starts with a read-only guest readback block and then prints the normal connection details. vm-summary always renders, even for partial step windows. For a destructive rebuild, run delete first and then create again.'
            )
            return
        }
        'update' {
        ...
        'task' {
            Write-AzVmHelpLines @(
                ...
                'Notes: --list is local-only and scans tracked plus local task trees with the same discovery rules used by init/update execution. --run-vm-init uses Azure run-command and replays the full guest transcript after each task completes. --run-vm-update uses the SSH task runner and streams guest stdout/stderr live; it still honors task-signaled immediate restarts, it does not perform the workflow-only final vm-update restart, and Windows Store-backed isolated reruns append `10003-create-public-desktop-shortcuts` so Public Desktop shortcuts refresh in the same task-run path. --save-app-state defaults to --source=vm and --restore-app-state defaults to --target=vm. Both default to --user=.all.. VM save/restore is SSH-based and remains limited to the managed manager/assistant profiles; vm-init app-state replay defers until SSH is ready. Local save/restore is Windows-host-only, reads or deploys only `<task-folder>/app-state/app-state.zip`, and writes a restore journal plus backup root before local replay.'
            )
            return
        }
        'connect' {
