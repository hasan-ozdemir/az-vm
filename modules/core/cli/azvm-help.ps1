    ...
    switch ($topicName) {
        ...
        'task' {
            Write-AzVmHelpLines @(
                ...
                'Notes: --list is local-only and scans tracked plus local task trees with the same discovery rules used by init/update execution. --run-vm-init uses Azure run-command. --run-vm-update uses the SSH task runner. --save-app-state defaults to --source=vm and --restore-app-state defaults to --target=vm. Both default to --user=.all.. VM save/restore remains limited to the managed manager/assistant profiles. Local save/restore is Windows-host-only, reads or deploys only `<task-folder>/app-state/app-state.zip`, and writes a restore journal plus backup root before local replay.'
            )
            return
        }
        'connect' {
