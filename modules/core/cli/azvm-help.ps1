    ...
    switch ($topicName) {
        ...
        'task' {
            Write-AzVmHelpLines @(
                ...
                'Notes: --list is local-only and scans tracked plus local task trees with the same discovery rules used by init/update execution. --run-vm-init uses Azure run-command and replays the full guest transcript after each task completes. --run-vm-update uses the SSH task runner and streams guest stdout/stderr live; it still honors task-signaled immediate restarts, it does not perform the workflow-only final vm-update restart, and Windows Store-backed isolated reruns append `10003-create-shortcuts-public-desktop` so Public Desktop shortcuts refresh in the same task-run path. --save-app-state defaults to --source=vm and --restore-app-state defaults to --target=vm. Both default to --user=.all.. VM save/restore is SSH-based and remains limited to the managed manager/assistant profiles; vm-init app-state replay defers until SSH is ready. Local save/restore is Windows-host-only, reads or deploys only `<task-folder>/app-state/app-state.zip`, and writes a restore journal plus backup root before local replay.'
            )
            return
        }
        'connect' {
