...
function Show-AzVmCommandHelpOverview {
    ...
    Write-Host "  create  Create or reuse managed resources and run VM init/update flow."
    Write-Host "  update  Re-run create-or-update operations against one existing managed VM."
    ...
    Write-Host "  az-vm update --step=network --auto"
...
function Show-AzVmCommandHelpDetailed {
    ...
    if ([string]::IsNullOrWhiteSpace($topicName)) {
        ...
        Write-Host "  create  : supports --step-to, --step-from, --step, explicit destructive rebuild flow"
        Write-Host "  update  : supports --step-to, --step-from, --step"
        ...
        Write-Host "  az-vm update --step-to=vm-init --auto"
        Write-Host "  az-vm group --list=<vm-name>"
        Write-Host "  az-vm group --select=<resource-group>"
        Write-Host "  az-vm do --vm-action=status --vm-name=<vm-name>"
        Write-Host "  az-vm do --vm-action=reapply --group=<resource-group> --vm-name=<vm-name>"
    ...
    switch ($topicName) {
        'create' {
            ...
            Write-Host "Description: create a managed VM deployment, reuse existing managed resources by default, and continue with VM init/update flow."
            Write-Host "Usage:"
            Write-Host "  az-vm create [--auto] [--windows|--linux] [--vm-name=<vm-name>] [--perf]"
            Write-Host "  az-vm create [--auto] [--windows|--linux] [--vm-name=<vm-name>] explicit destructive rebuild flow"
            ...
            Write-Host "  az-vm create explicit destructive rebuild flow --auto --windows"
            Write-Host "  az-vm create --step=network --linux"
            Write-Host "  az-vm create --step-from=vm-deploy --step-to=vm-summary --perf"
            Write-Host "Notes: create keeps existing managed resources by default. Use explicit destructive rebuild flow only when you want a destructive recreate."
            return
        }
        'update' {
            Write-Host "Command: update"
            Write-Host "Description: re-run create-or-update operations against one existing managed VM."
            Write-Host "Usage:"
            ...
            Write-Host "  az-vm update --step=vm-update --auto --windows"
            Write-Host "  az-vm update --step-from=group --step-to=vm-init --perf"
            Write-Host "Notes: update requires an existing managed resource group and an existing managed VM. In update mode, existing VMs are redeployed after Azure create-or-update."
            return
        }
        'configure' {
        ...
        'resize' {
            ...
            Write-Host "  az-vm resize"
            Write-Host "Notes: resize stays in the current region. --vm-size changes the VM SKU. --disk-size with --expand performs a supported managed OS disk growth. --disk-size with --shrink is a non-mutating guidance path that explains Azure's OS disk shrink limits and lists supported alternatives. If resize values are omitted, the command selects the managed group, VM, and VM SKU interactively."
            return
        }
        'set' {
