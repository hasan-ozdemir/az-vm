    ...
    try {
        ...
        if ($deallocated) {
            try {
                ...
                Write-Host ("VM '{0}' was started after the hibernation check." -f $vmName) -ForegroundColor DarkCyan
            }
            catch {
                Write-Warning ("VM '{0}' could not be started after the hibernation check: {1}" -f $vmName, $_.Exception.Message)
            }
        }
    }
